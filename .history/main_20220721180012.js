...
addBtnElement.addEventListener("click", function () {
  ...
  let t
  if (taskId == 0 || taskId) {
    tasks[id].name = { name: inputElement.value };
...
function renderTasks(tasks = []) {
  ...
  tasks.forEach((task, index) => {
    content += `<li class="item">
                  ${task.name}
                  <a class="edit-btn" onclick="editTask(${index})">Edit</a>
                  <a class="delete-btn" onclick="deleteTask(${index})">Delete</a>
                </li>`;
  });
...
function editTask(id) {
  ...
  addBtnElement.setAttribute("id", id);
  addBtnElement.innerText = "Save";
}
